...
def GridCreator(lower_limit,upper_limit,tshape,dim):
    T=CartesianGrid(dim,tshape,lower_limit,upper_limit)
    for i in range (dim):
        T.append(np.linspace(T.lower_limit[i],T.upper_limit[i],T.tshape[i]))
    return T
